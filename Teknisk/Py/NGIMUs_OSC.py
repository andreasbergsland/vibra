...
sens_arr = np.zeros((Active_NGIMUs, 10, HopSize))
batt_arr = np.zeros((Active_NGIMUs, 2))
rssi_arr = np.zeros((Active_NGIMUs, 2, HopSize))
alt_arr = np.zeros((Active_NGIMUs, 1, HopSize))
...
def euler(data, reader, time):
    global eul_cnt, HopSize, ontime, time_win, data_win, slope_list
...
def sensors(data, reader, time):
    NG_num = NGIMUs.index(reader)
    sens_arr[NG_num][0] = data[0]
    sens_arr[NG_num][1] = data[1]
    sens_arr[NG_num][2] = data[2]
    sens_arr[NG_num][3] = data[3]
    sens_arr[NG_num][4] = data[4]
    sens_arr[NG_num][5] = data[5]
    sens_arr[NG_num][6] = data[6]
    sens_arr[NG_num][7] = data[7]
    sens_arr[NG_num][8] = data[8]
    sens_arr[NG_num][9] = data[9]
    print(data, reader)

def rssi(data, reader, time):
    NG_num = NGIMUs.index(reader)
    rssi_arr[NG_num][0] = data[0]
    rssi_arr[NG_num][1] = data[1]
    print(data, reader)

def altitude(data, reader, time):
    NG_num = NGIMUs.index(reader)
    alt_arr[NG_num][0] = data[0]
    print(data, reader)
...
# NGIMU settings
osc_broadcast_client("255.255.255.255", 9000, "to_NGIMUs")  # Make broadcast client send bundle with settings.
...
RateBundle = oscbuildparse.OSCBundle(oscbuildparse.OSC_IMMEDIATELY,[EulerRate, QuatRate, LinRate, EarthRate, SensRate, AltRate, BattRate, RssiRate])
...
osc_method("/altitude", altitude, argscheme=osm.OSCARG_DATA + osm.OSCARG_READERNAME + osm.OSCARG_READTIME)
osc_method("/battery", battery, argscheme=osm.OSCARG_DATA + osm.OSCARG_READERNAME + osm.OSCARG_READTIME)
osc_method("/rssi", rssi, argscheme=osm.OSCARG_DATA + osm.OSCARG_READERNAME + osm.OSCARG_READTIME)
