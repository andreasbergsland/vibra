...
kMaxNum	=		20		; Number of bins to iterate through
giMaxAmps	ftgen		0, 0, 64, -7, 0
giMaxFrqs	ftgen		0, 0, 64, -7, 0
...
kcnt		init		0
kcnt		+=		knewflag
kWriteInt	=		30		; Interval in frames between each writing
if	kWriteInt >= 2	then
...
;   event	"i", 4, 0, 0.5, kmaxamp, kmaxfrq
  endif
;	printk2	kmaxind
;	printk2	kmaxamp, 20
;	printk2	kmaxcnt, 50
...
kInit		=		1
if	kInit	== 1	then
;tablecopy iFrqsTab, giMaxFrqs
;tablecopy iAmpsTab, giMaxAmps
endif
kInit		=		0
;iFreqFact	=		p5/(sr/2)
;iFade	table			iFreqFact, giFadePart, 1
kenv	cosseg		0.1, p3, 0
kenv	port			kenv, 0.005
aenv	=			a(kenv)
;aenv	expseg		0.00001, 0.01, 1, p3-0.01, 0.00001
;aSnd	oscil			p4 *aenv*0.2, p5, giSine
ares 		adsynt2 	kenv, 1, giSine,  giMaxFrqs, giMaxAmps, 20
;a1, a2	pan2		aSnd, iFreqFact*10
outs	ares, ares
